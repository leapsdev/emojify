...
export function useFarcasterAuth() {
  ...
  // ウォレットアドレス変更を監視してFirebase再認証を実行
  useEffect(() => {
    // MiniApp環境でない場合はスキップ
    if (!state.isMiniApp || !state.isSDKLoaded || !state.isReady) {
      return;
    }

    // Farcaster認証が成功していない場合はスキップ
    if (state.isFarcasterAuthenticated !== true) {
      return;
    }
    ...
    const checkWalletChange = async () => {
      try {
        ...
        try {
          ...
          console.error(
            '[useFarcasterAuth] Failed to get wallet address:',
            error,
          );
          return;
        }

        // Firebase認証済みユーザーのUIDと比較
        if (state.user && currentAddress && state.user.uid !== currentAddress) {
          console.log(
            '[useFarcasterAuth] Wallet address changed, re-authenticating:',
            {
              oldUid: state.user.uid,
        ...
        console.error('[useFarcasterAuth] Wallet change check error:', error);
      }
    };

    // 定期的にウォレットアドレスをチェック（5秒ごと）
