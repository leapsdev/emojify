    ...
    ReentrancyGuardUpgradeable
{
    ...
    function addEmojiSupply(
        ...
        if (_msgSender() != creator) {
            ...
            _distributeFees(creator, msg.value);
        }
    ...
    function addEmojiSupplyBatch(
        ...
        for (uint256 i = 0; i < emojiIds.length; i++) {
            address itemCreator = firstMinter[emojiIds[i]];
            if (itemCreator == address(0)) {
                revert EmojiNotRegistered(emojiIds[i]);
            }
            if (sender != itemCreator) {
                totalPaymentRequired += mintPrice * quantities[i];
            }
        }

        if (totalPaymentRequired > 0) {
            if (msg.value != totalPaymentRequired) {
                revert IncorrectPaymentValue(totalPaymentRequired, msg.value);
            }
        } else {
            if (msg.value != 0) {
                revert IncorrectPaymentValue(0, msg.value);
            }
        }
        ...
        if (totalPaymentRequired > 0) {
            uint256 paymentProcessed = 0;
            for (uint256 i = 0; i < emojiIds.length; i++) {
                address itemCreator = firstMinter[emojiIds[i]];
                if (
                    sender != itemCreator && quantities[i] > 0 && mintPrice > 0
                ) {
                    uint256 itemPaymentValue = mintPrice * quantities[i];
                    if (paymentProcessed + itemPaymentValue <= msg.value) {
                        _distributeFees(itemCreator, itemPaymentValue);
                        paymentProcessed += itemPaymentValue;
                    } else {
                        revert("Fee distribution inconsistency");
                    }
                }
            }
        }
    ...
    function _distributeFees(address creator, uint256 paymentAmount) internal {
        if (paymentAmount == 0) {
            return;
        }

        uint256 protocolFee = (paymentAmount * protocolFeePercent) / 100;
        uint256 creatorFee = paymentAmount - protocolFee;
